...
std::ostream&
operator<<( std::ostream& o, const QString& s )
{
    o << s.constData();
    return o;
}
...
{
    std::cerr << "YAML error " << e.what() << "in" << label << '.' << std::endl;
...
explainYamlException( const YAML::Exception& e, const QByteArray& yamlData )
{
    if ( ( e.mark.line >= 0 ) && ( e.mark.column >= 0 ) )
    {
        ...
        if ( linestart >= 0 )
            std::cerr << "offending YAML data:" << yamlData.mid( rangestart, rangeend-rangestart ).constData() << std::endl;
